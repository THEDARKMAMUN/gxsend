...
# Mapping of API numbers to functions, usable both by CLI and Flask.
    ...
    return jsonify(
        {
            ...
            "endpoints": {
                "GET /run-all?phone=...": {},
                "GET /run?phone=...&apis=1,2,51": {},
                "POST /run-all": {"phone": "string"},
                "POST /run": {
                    "phone": "string",
...
def http_run_selected():
    ...
    # Run selected APIs concurrently (each in a thread) for maximum speed.
    results = asyncio.run(_run_apis_concurrently(phone, api_numbers))
...
if __name__ == "__main__":
    # Start Flask server when run as a script.
    # Example:
    #   http://127.0.0.1:5000/run-all?phone=01313649970
    #   http://127.0.0.1:5000/run?phone=01313649970&apis=1,2,51
